...
# Initialize DuckDB (simple approach without WAL)
def init_duckdb():
...
def write_to_duckdb(analytics):
    # Create a new connection for each write (simple approach)
    con = duckdb.connect(DUCKDB_PATH)
    try:
        con.execute("""
...
def main():
    ...
    print("Note: Using simple mode - you can read data between writes!")
